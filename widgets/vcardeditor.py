...
class vcardEditorDialog(QtGui.QDialog):
	def __init__(self,main,data,parent=None):
		...
		if data.has_key("GIVEN"):
			self.ui.name.setText(data['GIVEN'])
		if data.has_key("FAMILY"):
			self.ui.surname.setText(data['FAMILY'])
	...
	def accept(self):
		self.data["GIVEN"]=unicode(self.ui.name.text())
		self.data["FAMILY"]=unicode(self.ui.surname.text())
		avatar=self.ui.avatar.pixmap()
		bytes=QtCore.QByteArray()
		buf=QtCore.QBuffer(bytes)
		buf.open(QtCore.QIODevice.WriteOnly)
		avatar.save(buf, "PNG")
		self.data["BINVAL"]=base64.encodestring(str(bytes))
		print self.data
