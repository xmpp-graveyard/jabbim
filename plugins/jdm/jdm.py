...
##		self.window.ui.esWidget.hide()
	...
	def setStatsPublic(self,data,widget):
		data=data[0][0]
		if len(data)>0:
			widget.addInfoText("jdm_public",widget.name+" "+unicode(self.tr("has %s files in Public Disc." % str(len(data)))))

	def setStats(self,data,widget,w):
		data=data[0][0]
		if len(data)>0:
			widget.addInfoText("jdm_album",widget.name+" "+unicode(self.tr("has %s photos in his album." % str(len(data)))))
