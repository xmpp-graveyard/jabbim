		...
		if self.config['on_first_message']['value']=="True":
			...
			if tab!=None:
				if tab.chat.first==None or tab.chat.first==True:
					...
					print unicode(user)
					self.main.events.addInfoEvent(header=self.tr("New message"),text=self.tr("From: ")+unicode(user),name=unicode(frm),typ='newMessage',icon="images/16x16/actions/message.png",action=self.main.chat.activate,actionDict=[])
					#self.startTrayBlink()
				else:
					print "pyco coe?"
					self.playsound('message')
