...
instance main = Main class instance
unicode id = iq id attribute that will be used in result
unicode session = parent Session instance
dict data = generaly data generated by x2dict
unicode xmllang = iq xml:lang attribute value
"""
		self.main = main
...
class Session:
	...
	def __init__(self, main, node, firststage, jid, sessionid, fid):
		"""instance main = Main instance
unicode node = name of the session node
class firststage = Stage class to be executed as first, next stages are defined in Stage.actions
unicode jid = full jid
unicode sessionid = generated automaticaly
unicode fid = first iq id attribute (for result of firststage)"""
		self.main	= main
...
unicode name = name of the node
unicode desc = human-readable name of the node
unicode jid = jid of node, if not set it is the same as client jid
bool public = if other jids (not only resources) can execute the node"""
		if jid == None:
